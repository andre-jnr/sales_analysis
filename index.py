...
def graph5(month, toggle):
    ...
    fig5.add_trace(go.Indicator(mode='number+delta',
                                title={"text": f"<span>{
                                    df_5['Consultor'].iloc[0]} - Top Consultant</span><br><span style='font-size:70%'>Em vendas - em relação a média</span><br>"},
    ...
    fig6.add_trace(go.Indicator(mode='number+delta',
                                title={"text": f"<span>{
                                    df_6['Equipe'].iloc[0]} - Top Team</span><br><span style='font-size:70%'>Em vendas - em relação a média</span><br>"},
...
def graph7(toggle):
    ...
    fig7.update_layout(main_config, yaxis={'title': None}, xaxis={
                       'title': None}, height=210, template=template)
...
def graph8(month, toggle):
    ...
    fig8.update_layout(main_config, height=450, template=template)
    return fig8
